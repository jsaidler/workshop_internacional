...
function cms_validate_form_schema(array $schema): array {
    ...
    $fieldIds=array_fill_keys(array_column($fields,'id'),true);
    $settings=is_array($schema['settings']??null)?$schema['settings']:[];
    $blocks=[];$blockIds=[];
    foreach(($settings['contentBlocks']??[]) as $raw){
        if(!is_array($raw))continue;
        $id=cms_form_field_id((string)($raw['id']??$raw['label']??'content'));
        if(isset($blockIds[$id]))$id.='_'.(count($blocks)+1);
        $blockIds[$id]=true;
        $after=trim((string)($raw['afterField']??''));if($after!==''&&!isset($fieldIds[$after]))$after='';
        $block=[
            'id'=>$id,
        ...
        if($source!==''&&isset($fieldIds[$source])){
            $operator=(string)($condition['operator']??'equals');
            if(!in_array($operator,['equals','not_equals','contains','checked','not_checked'],true))$operator='equals';
            $value=in_array($operator,['checked','not_checked'],true)?'':trim((string)($condition['value']??''));
            $block['condition']=['source'=>$source,'operator'=>$operator,'value'=>$value];
        }
        $blocks[]=$block;
    }
    $settings['contentBlocks']=$blocks;
...
function cms_form_by_key(PDO $db,int $activityId,string $locale,string $key): ?array {$q=$db->prepare("SELECT * FROM cms_forms WHERE activity_id=? AND locale=? AND form_key=? AND status!='archived'");$q->execute([$activityId,$locale,$key]);return $q->fetch()?:null;}
...
function cms_render_form(array $form,array $schema,int $pageId,string $locale,array $values=[],array $errors=[],bool $success=false,bool $editor=false): string {
    ...
    foreach($byAfter['']??[] as $block)$html.=cms_form_content_block_html($block,$editor);
    foreach($schema['fields'] as $field){$html.=cms_form_input_html($field,$values[$field['id']]??null);if(isset($errors[$field['id']]))$html.='<p class="form-error">'.h($errors[$field['id']]).'</p>';foreach($byAfter[(string)$field['id']]??[] as $block)$html.=cms_form_content_block_html($block,$editor);}
    $html.='</div><button class="button" type="'.($editor?'button':'submit').'">'.h($schema['submitLabel']).' <span aria-hidden="true">↗</span></button></form>';return $html;
}
