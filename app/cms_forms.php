...
function cms_form_field_id(string $value): string {
    ...
    $value=preg_replace('~[^a-z0-9_]+~','_',iconv('UTF-8','ASCII//TRANSLIT',$value)?:$value)??'';
...
function cms_forms_seed(PDO $db,int $activityId): void {
    ...
    $now=utc_now();
    $seeds=[
        [PUBLIC_LOCALE_PT_BR,'registration','Inscrição — nova turma',cms_form_schema_default(PUBLIC_LOCALE_PT_BR,'registration')],
        [PUBLIC_LOCALE_EN,'interest','Interest survey — English cohort',cms_form_schema_default(PUBLIC_LOCALE_EN,'interest')],
    ];
    ...
    foreach($seeds as [$locale,$key,$title,$schema]){$json=json_encode(cms_validate_form_schema($schema),JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);$uuid=cms_form_uuid();$insert->execute([$uuid,$activityId,$locale,$key,$title,$json,$json,$now,$now,$now,$now]);}
}
function cms_forms(PDO $db,int $activityId,?string $locale=null,bool $includeArchived=false): array {
    cms_forms_seed($db,$activityId);$sql='SELECT * FROM cms_forms WHERE activity_id=?';$args=[$activityId];if($locale!==null){$sql.=' AND locale=?';$args[]=$locale;}if(!$includeArchived)$sql.=" AND status!='archived'";$sql.=' ORDER BY locale,title,id';$q=$db->prepare($sql);$q->execute($args);return $q->fetchAll();
}
...
function cms_form_create(PDO $db,int $activityId,string $locale,string $title,string $key,string $kind='interest'): array {
    $locale=normalize_public_locale($locale)??PUBLIC_LOCALE_EN;$key=activity_slug($key);$base=$key;$n=2;while(cms_form_by_key($db,$activityId,$locale,$key))$key=$base.'-'.$n++;
    $schema=cms_validate_form_schema(cms_form_schema_default($locale,$kind));$json=json_encode($schema,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);$now=utc_now();$q=$db->prepare('INSERT INTO cms_forms(form_uuid,activity_id,locale,form_key,title,status,draft_schema_json,published_schema_json,draft_revision,published_revision,draft_updated_at,published_at,created_at,updated_at)VALUES(?,?,?,?,?,"active",?,?,1,1,?,?,?,?)');$q->execute([cms_form_uuid(),$activityId,$locale,$key,trim($title)?:$key,$json,$json,$now,$now,$now,$now]);return cms_form_by_id($db,(int)$db->lastInsertId())??throw new RuntimeException('form_create_failed');
}
function cms_form_save(PDO $db,int $id,array $schema,int $revision,?string $title=null): array {
    $form=cms_form_by_id($db,$id)??throw new RuntimeException('form_not_found');$schema=cms_validate_form_schema($schema);$json=json_encode($schema,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);$new=$revision+1;$now=utc_now();$q=$db->prepare('UPDATE cms_forms SET title=COALESCE(?,title),draft_schema_json=?,draft_revision=?,draft_updated_at=?,updated_at=? WHERE id=? AND draft_revision=?');$q->execute([$title===null?null:trim($title),$json,$new,$now,$now,$id,$revision]);if($q->rowCount()!==1)throw new RuntimeException('revision_conflict');return cms_form_by_id($db,$id)??throw new RuntimeException('form_not_found');
}
...
function cms_form_field_value(array $field,array $input): mixed {
    $id=$field['id'];$type=$field['type'];$value=$input[$id]??($type==='checkbox-group'?[]:'');
    if($type==='checkbox-group'){if(!is_array($value))$value=[];return array_values(array_map('strval',$value));}
    if(in_array($type,['checkbox','consent'],true))return !empty($value)?'1':'';
    return is_array($value)?'':trim((string)$value);
}
function cms_form_validate_submission(array $schema,array $input): array {
    $values=[];$errors=[];
    foreach($schema['fields'] as $field){
        $id=$field['id'];$type=$field['type'];$value=cms_form_field_value($field,$input);$values[$id]=$value;
        $empty=is_array($value)?count($value)===0:$value==='';
        if(!empty($field['required'])&&$empty){$errors[$id]='Campo obrigatório.';continue;}
        if($type==='email'&&!$empty&&!filter_var($value,FILTER_VALIDATE_EMAIL))$errors[$id]='Informe um e-mail válido.';
        if(in_array($type,['select','radio'],true)&&!$empty){$allowed=array_column($field['options']??[],'value');if(!in_array($value,$allowed,true))$errors[$id]='Valor inválido.';}
        if($type==='checkbox-group'&&!$empty){$allowed=array_column($field['options']??[],'value');foreach($value as $item)if(!in_array($item,$allowed,true)){$errors[$id]='Valor inválido.';break;}}
        if(is_string($value)&&strlen($value)>12000)$errors[$id]='Conteúdo muito longo.';
    }
    return [$values,$errors];
}
function cms_submission_save(PDO $db,array $form,array $schema,array $values,?int $pageId,string $locale,string $sourceUrl=''): string {
    $config=app_config();$ip=hash_hmac('sha256',$_SERVER['REMOTE_ADDR']??'',$config['ip_hash_secret']);$ua=hash_hmac('sha256',$_SERVER['HTTP_USER_AGENT']??'',$config['app_secret']);$uuid=cms_form_uuid();$now=utc_now();
    $db->beginTransaction();try{rate_limit_or_throw($db,$ip,$config);$q=$db->prepare('INSERT INTO cms_form_submissions(submission_uuid,form_id,page_id,activity_id,locale,payload_json,form_snapshot_json,source_url,status,notes,consent,ip_hash,user_agent_hash,created_at,updated_at)VALUES(?,?,?,?,?,?,?,?,"new","",?,?,?,?,?)');$consent=0;foreach($schema['fields'] as $field)if($field['type']==='consent'&&!empty($values[$field['id']])){$consent=1;break;}$q->execute([$uuid,(int)$form['id'],$pageId,(int)$form['activity_id'],$locale,json_encode($values,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),json_encode($schema,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),substr($sourceUrl,0,1000),$consent,$ip,$ua,$now,$now]);$db->commit();return $uuid;}catch(Throwable $e){if($db->inTransaction())$db->rollBack();throw $e;}
}
...
function cms_render_form(array $form,array $schema,int $pageId,string $locale,array $values=[],array $errors=[],bool $success=false,bool $editor=false): string {
    ...
    if($success)return '<div class="cms-form-success" id="'.$anchor.'"><p class="section-label">'.h($form['title']).'</p><h2>'.h($schema['successTitle']).'</h2><p>'.h($schema['successMessage']).'</p></div>';
    $html='<form class="cms-form" id="'.$anchor.'" action="/form-submit.php" method="post"'.($editor?' data-cms-form-preview="1"':'').'>';
    if(!$editor){$html.='<input type="hidden" name="_csrf" value="'.h(csrf_token('cms-form')).'"><input type="hidden" name="_form_uuid" value="'.h($form['form_uuid']).'"><input type="hidden" name="_page_id" value="'.$pageId.'"><input type="hidden" name="_locale" value="'.h($locale).'"><input type="hidden" name="_started_at" value="'.time().'"><label class="honeypot" aria-hidden="true">Leave blank<input name="_website" tabindex="-1" autocomplete="off"></label>';}
