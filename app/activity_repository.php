...
function activity_update(PDO $db,int $id,string $adminName,string $publicTitle,string $slug): array {
    $activity=activity_by_id($db,$id)??throw new RuntimeException('Site não encontrado.');
    $adminName=trim($adminName);$publicTitle=trim($publicTitle);$slug=activity_slug($slug);
    if($adminName===''||$publicTitle==='')throw new RuntimeException('Informe o nome administrativo e o nome público do curso.');
    $q=$db->prepare('SELECT id FROM activities WHERE slug=? AND id!=? LIMIT 1');$q->execute([$slug,$id]);
    if($q->fetchColumn())throw new RuntimeException('Já existe outro site com este slug.');
    $db->prepare('UPDATE activities SET admin_name=?,public_title=?,slug=?,updated_at=? WHERE id=?')->execute([$adminName,$publicTitle,$slug,gmdate('c'),$id]);
