...
const span=[...automatic,['1','1 coluna'],['2','2 colunas'],['3','3 colunas'],['4','4 colunas']];
const mobileSpan=[...automatic,['1','1 coluna'],['2','2 colunas']];
...
function columnPanel(node){
  const box=document.createElement('div');box.className='cms-layout-properties cms-pro-panel';
  box.innerHTML=`<hr><p class="eyebrow">Coluna</p><h3>Desktop</h3><div class="cms-layout-property-grid">${field('Largura na grade','cmsSpan',node.dataset.cmsSpan||'',span)}${field('Conteúdo vertical','cmsJustify',node.dataset.cmsJustify||'',justify)}${field('Conteúdo horizontal','cmsColumnAlign',node.dataset.cmsColumnAlign||'',items)}${field('Texto','cmsTextAlign',node.dataset.cmsTextAlign||'',textAlign)}${field('Ordem','cmsOrder',node.dataset.cmsOrder||'',order)}</div>${checkbox('Ocultar no desktop','cmsHideDesktop',node.dataset.cmsHideDesktop==='1')}<h3>Tablet</h3><div class="cms-layout-property-grid">${field('Largura na grade','cmsTabletSpan',node.dataset.cmsTabletSpan||'',span)}${field('Conteúdo vertical','cmsTabletJustify',node.dataset.cmsTabletJustify||'',justify)}${field('Conteúdo horizontal','cmsTabletColumnAlign',node.dataset.cmsTabletColumnAlign||'',items)}${field('Texto','cmsTabletTextAlign',node.dataset.cmsTabletTextAlign||'',textAlign)}${field('Ordem','cmsTabletOrder',node.dataset.cmsTabletOrder||'',order)}</div>${checkbox('Ocultar no tablet','cmsHideTablet',node.dataset.cmsHideTablet==='1')}<h3>Celular</h3><div class="cms-layout-property-grid">${field('Largura na grade','cmsMobileSpan',node.dataset.cmsMobileSpan||'',mobileSpan)}${field('Conteúdo vertical','cmsMobileJustify',node.dataset.cmsMobileJustify||'',justify)}${field('Conteúdo horizontal','cmsMobileColumnAlign',node.dataset.cmsMobileColumnAlign||'',items)}${field('Texto','cmsMobileTextAlign',node.dataset.cmsMobileTextAlign||'',textAlign)}${field('Ordem','cmsMobileOrder',node.dataset.cmsMobileOrder||'',order)}</div>${checkbox('Ocultar no celular','cmsHideMobile',node.dataset.cmsHideMobile==='1')}<p class="inspector-note">A largura ocupa colunas do grupo naquele tamanho de tela. “Automático” ocupa uma coluna; quando o grupo está em uma única coluna, o conteúdo permanece em largura total. Ordem e visibilidade também são independentes por dispositivo.</p>`;
  bind(box,node);return box;
}
