...
O cliente registra apenas links de ação relevantes (`a.button`, CTA de cabeçalho e barra persistente), ignorando âncoras locais. O evento guarda destino, texto curto do CTA e posição (barra persistente, cabeçalho, hero ou nome da seção quando disponível).
