...
<div class="analytics-grid">
  <div>
    ...
    <section class="analytics-panel">
      ...
      <?php if(!$data['sources']):?><p class="analytics-empty">As origens aparecerão quando houver acessos.</p><?php else:?><div style="overflow:auto"><table class="analytics-table"><thead><tr><th>Origem</th><th>Campanha</th><th class="num">Sessões</th></tr></thead><tbody><?php foreach($data['sources'] as $row):?><tr><td class="analytics-source"><strong><?=h((string)$row['source'])?></strong><?php if((string)$row['medium']!==''):?><small><?=h((string)$row['medium'])?></small><?php endif;?></td><td><?=h((string)$row['campaign']?:'—')?></td><td class="num"><?=number_format((int)$row['sessions'],0,',','.')?></td></tr><?php endforeach;?></tbody></table></div><?php endif;?>
    </section>
  </div>
  <aside>
