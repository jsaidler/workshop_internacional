...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
    ...
    $now=utc_now();$paymentNote=trim((string)($_POST['payment_note']??''));
...
<section class="inbox-toolbar"><div><h2>Inscrições</h2><p><?=$counts['new']?> aguardando · <?=$counts['converted']?> confirmada<?=$counts['converted']===1?'':'s'?></p></div><div class="inbox-toolbar-actions"><a href="/admin/forms.php?activity=<?=$activityId?>">Editar formulário</a><a href="/admin/submissions-export.php?activity=<?=$activityId?><?=$formFilter?'&form='.$formFilter:''?>">Exportar CSV</a></div></section>
<?php if(count($forms)>1):?><form class="inbox-form-filter" method="get"><input type="hidden" name="activity" value="<?=$activityId?>"><label>Formulário <select name="form" onchange="this.form.submit()"><option value="0">Todos</option><?php foreach($forms as $form):?><option value="<?=(int)$form['id']?>"<?=$formFilter===(int)$form['id']?' selected':''?>><?=h($form['title'].' · '.public_language_label($form['locale']))?></option><?php endforeach;?></select></label></form><?php endif;?>
...
<section class="registration-admin-status <?=$selected['payment_status']==='paid'?'is-paid':'is-pending'?>"><span>Inscrição</span><strong><?=h(registration_status($selected))?></strong></section>
<section class="registration-admin-group"><h3>Contato</h3><dl class="inbox-fields"><?php foreach(['cpf'=>'CPF','phone'=>'WhatsApp','email'=>'E-mail','instagram'=>'Instagram'] as $key=>$label):if(trim((string)($payload[$key]??''))==='')continue;?><div><dt><?=h($label)?></dt><dd><?=h((string)$payload[$key])?></dd></div><?php endforeach;?></dl></section>
<section class="registration-admin-group"><h3>Presente e envio</h3><dl class="inbox-fields"><?php foreach(['support_size'=>'Tamanho do suporte','address'=>'Endereço','city_state'=>'Cidade/UF','postal_code'=>'CEP'] as $key=>$label):$value=$payload[$key]??'';if($value===''||$value===[])continue;?><div><dt><?=h($label)?></dt><dd><?=h(submission_value_label($fields[$key]??[],$value))?></dd></div><?php endforeach;?></dl></section>
...
<link rel="stylesheet" href="/assets/admin-inbox.css"><link rel="stylesheet" href="/assets/admin-registration.css">
<?php admin_shell_end();
