...
try{
    ...
    if(array_key_exists('visibility',$input)){
        ...
        if($visibility==='public'){$q=$db->prepare('SELECT 1 FROM course_page_media_slots WHERE media_asset_id=? LIMIT 1');$q->execute([$id]);if($q->fetchColumn())throw new RuntimeException('Remova esta mídia dos espaços protegidos antes de torná-la pública.');}
        $db->prepare('UPDATE media_assets SET visibility=?,updated_at=? WHERE id=?')->execute([$visibility,gmdate('c'),$id]);
    }
    $item=media_update_metadata($db,$id,$input);echo json_encode(['item'=>$item],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
