...
$local=$status['local']??update_local_info();$remote=$status['remote']??null;$available=(bool)($status['available']??false);$backups=update_backup_history();$health=system_health_collect();
...
<article class="overview-card health-summary health-<?=h($health['status'])?>"><p class="admin-kicker">Diagnóstico</p><h2><?=$health['status']==='ok'?'Ambiente pronto':($health['status']==='warn'?'Atenção':'Intervenção necessária')?></h2><p><?=count($health['items'])?> verificações · <?=(int)$health['errors']?> erro(s) · <?=(int)$health['warnings']?> aviso(s)</p></article>
</section>
<section class="admin-editor-card"><div class="admin-editor-header"><div><p class="admin-kicker">Saúde do sistema</p><h2>Diagnóstico da hospedagem</h2></div><p>Verificação feita nesta requisição. Nenhuma credencial, caminho físico do servidor ou valor sensível é exibido.</p></div>
<div class="system-health-grid"><?php foreach($health['items'] as $item):?><article class="system-health-item health-<?=h((string)$item['status'])?>"><span class="health-dot" aria-hidden="true"></span><div><strong><?=h((string)$item['label'])?></strong><p><?=h((string)$item['detail'])?></p></div><span class="health-label"><?=($item['status']==='ok'?'OK':($item['status']==='warn'?'Atenção':'Erro'))?></span></article><?php endforeach;?></div>
<p class="admin-muted">A disponibilidade de <code>mail()</code> confirma apenas que o PHP oferece a função; não garante entrega de mensagens. Integridade do SQLite é verificada com <code>PRAGMA quick_check</code>.</p>
</section>
...
<style>.system-health-grid{display:grid;gap:8px}.system-health-item{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:12px;align-items:start;padding:14px 16px;border:1px solid rgba(127,127,127,.25);border-radius:8px}.system-health-item p{margin:.2rem 0 0}.health-dot{width:10px;height:10px;border-radius:50%;margin-top:.35rem;background:#777}.health-ok .health-dot{background:#247a45}.health-warn .health-dot{background:#a16a00}.health-error .health-dot{background:#a73333}.health-label{font-size:.72rem;text-transform:uppercase;letter-spacing:.08em;font-weight:700}.health-summary.health-error{border-color:#a73333}.health-summary.health-warn{border-color:#a16a00}@media(max-width:680px){.system-health-item{grid-template-columns:auto 1fr}.health-label{grid-column:2}}</style>
<?php admin_shell_end();
