...
<section class="student-section">
  ...
  <?php if(!$tests):?><div class="student-empty">Você ainda não registrou nenhum teste.</div><?php else:?><div class="student-test-list"><?php foreach($tests as $test):?><article class="student-test-item"><a class="student-test-row" href="/aluno/teste.php?id=<?=(int)$test['id']?>"><div><span class="student-status student-status-<?=h((string)$test['status'])?>"><?=h(student_test_status_label((string)$test['status']))?></span><h3><?=h((string)$test['title'])?></h3><p><?=h((string)$test['public_title'])?> · <?=h((string)$test['cohort_title'])?></p></div><div class="student-test-meta"><span><?=h((string)($test['test_date']?:'sem data'))?></span><span>Atualizado <?=h(student_ops_datetime_exists((string)$test['updated_at']))?></span></div></a><a class="student-test-delete-link" href="/aluno/excluir-teste.php?id=<?=(int)$test['id']?>">Excluir teste</a></article><?php endforeach;?></div><?php endif;?>
</section>
