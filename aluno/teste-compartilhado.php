...
<section class="student-workflow-panel">
  <div class="student-review-grid">
    ...
    <article><span>Revelação</span><dl><div><dt>Revelador</dt><dd><?=student_review_value($test['developer'])?></dd></div><div><dt>Diluição</dt><dd><?=student_review_value($test['dilution'])?></dd></div><div><dt>Temperatura</dt><dd><?=student_review_value($test['temperature'])?></dd></div><div><dt>Tempo</dt><dd><?=student_review_value($test['development_time'])?></dd></div><div><dt>Branqueador</dt><dd><?=student_review_value($test['bleach']??'')?></dd></div></dl><?php if(trim((string)$test['agitation'])!==''):?><p><?=nl2br(h((string)$test['agitation']))?></p><?php endif;?><?php if(trim((string)$test['notes'])!==''):?><p><?=nl2br(h((string)$test['notes']))?></p><?php endif;?></article>
  </div>
...
<section class="student-workflow-panel student-conversation">
  <header class="student-workflow-heading"><div><p class="student-kicker">Acompanhamento</p><h2 class="student-subtitle">Avaliação e dúvidas</h2></div><p>A conversa faz parte do registro e segue a mesma visibilidade do teste.</p></header>
  <?php if(!$messages):?><div class="student-empty">Ainda não há mensagens neste teste.</div><?php else:?><div class="student-thread"><?php foreach($messages as $message):?><article class="student-message <?=$message['author_role']==='admin'?'student-message-admin':'student-message-student'?>"><header><strong><?=$message['author_role']==='admin'?'João · avaliação':h((string)$test['student_name'])?></strong><span><?=h(student_test_message_date((string)$message['created_at']))?></span></header><p><?=nl2br(h((string)$message['body']))?></p></article><?php endforeach;?></div><?php endif;?>
</section>
