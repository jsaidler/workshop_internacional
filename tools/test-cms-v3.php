...
$fonts=cms_design_font_defaults();$defaults=cms_design_defaults();v3_expect($defaults['type']['bodyFont']==='IBM Plex Sans','default body font should be a Google Fonts family name');v3_expect($defaults['type']['displayFont']==='Saira Extra Condensed','default display font should be a Google Fonts family name');
$design=cms_settings_save($db,'design',1,PUBLIC_LOCALE_PT_BR,['type'=>['displayLineHeight'=>1.05,'bodyFont'=>'var(--sans)'],'layout'=>['maxWidth'=>1600],'advanced'=>['customCss'=>'.custom-test{display:block}']]);$css=cms_design_css($design);v3_expect($design['type']['bodyFont']===$fonts['body'],'legacy body font variable should normalize when Design is saved');v3_expect(str_contains($css,'--body:"IBM Plex Sans", Arial, sans-serif;'),'body font family should expand to the internal CSS stack');v3_expect(str_contains(cms_design_google_fonts_url($design),'family=IBM+Plex+Sans'),'selected body font must be requested from Google Fonts');v3_expect(str_contains($css,'--cms-display-lh:1.05'),'display line-height token missing');v3_expect(str_contains($css,'--max:1600px'),'max width token missing');v3_expect(str_contains($css,'prefers-color-scheme:dark'),'auto dark mode missing');v3_expect(str_contains($css,'[data-cms-span="2"]'),'grid span support missing');v3_expect(str_contains($css,'.custom-test{display:block}'),'custom CSS missing');
