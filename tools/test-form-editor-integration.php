...
must(str_contains($quick,"part:'field-control'"),'actual input/select/textarea controls must remain selectable as field context');
must(str_contains($quick,'decorateAll(d)'),'all rendered form sub-targets must be prepared when the preview document is installed');
must(str_contains($quick,'resolveTarget(event.target)'),'interaction must resolve the clicked descendant, not treat the form as one entity');
must(str_contains($quick,'.cms-choice-grid > label')&&str_contains($quick,'.cms-choice-group > legend')&&str_contains($quick,'.cms-field')&&str_contains($quick,'.cms-consent'),'resolver must cover visible field and option structures even when the exact nested span is not the event target');
must(str_contains($quick,"setAttribute('contenteditable','true')"),'visible form copy must become editable in place');
must(str_contains($quick,"addEventListener('pointerdown'")&&str_contains($quick,'stopImmediatePropagation'),'recognized form descendants must win before legacy wrapper handlers');
must(str_contains($quick,'neutralizeLegacyWrapperSelection'),'rendered forms must explicitly disable the obsolete whole-form selection fallback');
must(str_contains($quick,'block.onclick=null'),'expanded form wrappers must not send clicks back to the legacy form inspector');
must(str_contains($quick,'setTimeout(()=>neutralizeLegacyWrapperSelection(d),0)'),'legacy wrapper selection must be removed after the core load hook has had a chance to attach it');
...
must(str_contains($quick,'Configuração estrutural do formulário'),'full form builder must be an explicit secondary structural action, never the click fallback');
...
must(!str_contains($quick,"if(formBlockFrom(event.target)){event.preventDefault();event.stopImmediatePropagation()}"),'form controller must not blanket-suppress all clicks merely because they occur inside a form');
