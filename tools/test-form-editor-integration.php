...
must(str_contains($quick,'resolveEditableTarget'),'visual editing must resolve the clicked form text directly from the rendered page');
must(str_contains($quick,'.cms-choice-grid > label > span')&&str_contains($quick,'.cms-choice-group > legend')&&str_contains($quick,'.cms-field > span'),'radio/checkbox options and field labels must be discoverable as direct visual targets');
...
must(!str_contains($quick,'ensureForm(formId).then(()=>{if(selectedBlock()===block&&!selected)renderFormHint(block)})'),'loaded form state must never recursively render the same inspector forever');
