...
return static function(PDO $db): void {
    ...
    $rows=$db->query("SELECT id,draft_document_json,published_document_json,draft_revision,published_revision FROM cms_pages WHERE status!='archived'")->fetchAll(PDO::FETCH_ASSOC);$now=gmdate('c');
    foreach($rows as $row){$sets=[];$args=[];$draft=(string)($row['draft_document_json']??'');$published=(string)($row['published_document_json']??'');$newDraft=str_replace(array_keys($replacements),array_values($replacements),$draft,$draftCount);$newPublished=str_replace(array_keys($replacements),array_values($replacements),$published,$publishedCount);
        ...
        if(!$sets)continue;$sets[]='updated_at=?';$args[]=$now;$args[]=(int)$row['id'];$q=$db->prepare('UPDATE cms_pages SET '.implode(',',$sets).' WHERE id=?');$q->execute($args);
    }
};
